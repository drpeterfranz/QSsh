...
namespace Internal {
    ...
    SshAbstractCryptoFacility::Mode SshAbstractCryptoFacility::getMode(const QByteArray& algoName)
    {
        ...
        if (algoName.endsWith("-cbc"))
            return CbcMode;
        throw SshClientException(SshInternalError, SSH_TR("Unexpected cipher \"%1\"").arg(QString::fromLatin1(algoName)));
    }
    ...
    Keyed_Filter* SshEncryptionFacility::makeCipherMode(const QByteArray& cipher, const Mode mode)
    {
        ...
        Cipher_Mode_Filter* filter = new Cipher_Mode_Filter(
            Cipher_Mode::create_or_throw(cipher.toStdString(), Cipher_Dir::Encryption).release()); // We have to release, otherwise clang fails to link
        return filter;
    }
    ...
    void SshEncryptionFacility::createAuthenticationKey(const QByteArray& privKeyFileContents)
    {
        ...
        if (ecdsaKey) {
            ...
            m_authPubKeyBlob += AbstractSshPacket::encodeString(
                convertByteArray(ecdsaKey->public_point().encode(EC_Point_Format::Uncompressed)));
        } else {
    ...
    {
        try {
            std::string_view fileView(privKeyFileContents.begin(), privKeyFileContents.end());
            auto privKeySrc = DataSource_Memory(fileView);
            auto key = PKCS8::load_key(privKeySrc, SshKeyPasswordRetriever::get_passphrase);
            m_authKey.reset(key.release());
            if (auto* const dsaKey = dynamic_cast<DSA_PrivateKey*>(m_authKey.data())) {
                m_authKeyAlgoName = SshCapabilities::PubKeyDss;
                pubKeyParams << dsaKey->get_int_field("p") << dsaKey->get_int_field("q")
                             << dsaKey->get_int_field("g") << dsaKey->get_int_field("y");
                allKeyParams << pubKeyParams << dsaKey->get_int_field("x");
        ...
        QList<BigInt>& pubKeyParams, QList<BigInt>& allKeyParams, QString& error)
    {
        try {
            ...
            BER_Decoder sequence = decoder.start_cons(Botan::ASN1_Type::Sequence, Botan::ASN1_Class::Universal);
            size_t version;
            ...
            if (m_authKeyAlgoName == SshCapabilities::PubKeyDss) {
                ...
                DSA_PrivateKey* const dsaKey = new DSA_PrivateKey(DL_Group(p, q, g), x);
    ...
    QByteArray SshEncryptionFacility::authenticationKeySignature(const QByteArray& data) const
    {
        ...
        if (m_authKeyAlgoName.startsWith(SshCapabilities::PubKeyEcdsaPrefix)) {
            ...
            const int halfSize = signature.size() / 2;
    ...
    Keyed_Filter* SshDecryptionFacility::makeCipherMode(const QByteArray& cipher, const Mode mode)
    {
        ...
        Cipher_Mode_Filter* filter = new Cipher_Mode_Filter(
            Cipher_Mode::create_or_throw(cipher.toStdString(), Cipher_Dir::Decryption).release()); // We have to release, otherwise clang fails to link
        return filter;
    }
